...
(() => {
    ...
    rollData.TraitCombos
        ...
        .sort((b,a) => (
            ...
            leastFrequency = Math.min(leastFrequency, combo.Count);
        });
    ...
    for (let i = 0; i <= columnThreeNames.length; i++) {
        for (let j = 0; j <= columnFourNames.length; j++) {
            ...
            cellElement.style.backgroundColor = `rgba(82, 163, 71, ${(comboFrequency - leastFrequency) / (greatestFrequency - leastFrequency)})`;
            if (i === 0 && j === 0) {
